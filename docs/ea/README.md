...
## Lo primero: para abrir un `.eapx` en la web NO necesitás nada de esto

Arrastrá el `.eapx` al lienzo de la aplicación, o usá
`Intercambio > Importar > Desde Enterprise Architect`. El servidor lee el
archivo y te muestra los diagramas de clases que tiene adentro. Cero pasos en
EA.

Estos scripts son para el otro sentido y para armar la documentación.
...
| `ExportarDesdeEA.js` | EA → CASE | Alternativa por si preferís exportar un paquete suelto a JSON en vez de subir el `.eapx` completo. Normalmente no hace falta. |
